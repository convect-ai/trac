...
def list_datasets(request, instance_id):
    """
    Show a list of registered datasets when request is GET
    """
    datasets = DataSet.objects.filter(app__pk=instance_id).all()
    return render(
        request,
        "data_gateway/list_datasets.html",
        {"datasets": datasets, "instance_id": instance_id},
    )
...
def create_dataset(request, instance_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            dataset.save()
            return redirect("data_gateway:list_datasets", instance_id=instance_id)
    else:
        form = DatasetForm()
    return render(
        request,
